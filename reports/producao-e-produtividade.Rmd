---
title: "Regressão linear na prática - Lab 5.1 - FPCC2"
author: "Nisston Moraes Tavares de Melo - 0120015808-15D"
...
```{r}
modelo5 = lm(produtividade ~ docentes_visitantes + tem_doutorado + nivel, data = cacc)
...
produtividade = - 0,14.docentes_visitantes + 0,81.tem_doutorado + 0,59.nivel - 0,28

O modelo pode explicar 56,54% da variância da variável de resposta. Também é possível verificar que se o aumento de 1 (uma) unidade de docentes visistantes e nivel produz uma mudança em produtividade de, respectivamente, -0,14 e 0,59. No caso do programa de pós graduação ter doutorado (tem_doutorado = 1), será produzida uma mudança de 0,81 na produtividade. Os coeficientes angular da reta.
...
* A adição de docentes visitantes pode exercer nenhuma influência na produtividade, ou se ela existir pode ser relevante negativa ou razoável positiva (b1 = [-0,52;0,25], IC com 95%).

* Já o programa de pós-graduação em Ciência da Computação dispor de doutorado, exerce uma influência na produtividade que pode ser razoável positiva ou muito forte positiva (b2 = [0,22;1,40], IC com 95%).

* De igual modo, é possível inferir que as notas atribuídas aos programas de pós-graduação possuem uma relação média ou forte com a produtividade dos mesmos (b3 = [0,34;0,85], IC com 95%).
